...
class Posicionador {
	...
	method randomPosPara(unPersonaje) {
		unPersonaje.position(grillaDeJuego.unaPos())
	}
...
object posicionadorFantasma inherits Posicionador {
	...
	method filtrarPosUsablesEn(unasPos) {
		unasPos.removeAllSuchThat({ unaPos => !self.esPosUsable(unaPos)})
	}

	method nuevaPosPara(unEnemigo) {
		const movPos = [ unEnemigo.position().up(1), unEnemigo.position().down(1), unEnemigo.position().left(1), unEnemigo.position().right(1) ]
...
object posicionadorPacman inherits Posicionador {
	...
	method nuevaPosPara(unPersonaje) {
		if (self.esPosUsable(self.proxPosDe(unPersonaje))) unPersonaje.position(self.proxPosDe(unPersonaje))
	}
