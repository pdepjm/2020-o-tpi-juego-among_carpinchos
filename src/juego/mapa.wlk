...
import juego.objetos.muro.Wall

object generadorDeMuros {

	const muros = []

	method verticalWall(cantidad, inicio) {
		(0 .. cantidad - 1).forEach{ n => muros.add(new Wall(position = inicio.up(n)))}
	}

	method horizontalWall(cantidad, inicio) {
		(0 .. cantidad - 1).forEach{ n => muros.add(new Wall(position = inicio.right(n)))}
	}
