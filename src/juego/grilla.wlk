...
object grillaDeJuego {
	...
	method filtrarPosLibresEn(unaGrilla) {
		unaGrilla.removeAllSuchThat({ unaPos => !self.esUnaPosLibre(unaPos)})
	}

	method generarGrillaDe(ancho, alto) {
		(1 .. ancho - 1).forEach{ x => (1 .. alto - 1).forEach{ y => grilla.add(game.at(x, y))}}
